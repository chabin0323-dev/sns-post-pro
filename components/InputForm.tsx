...
function scoreSuggestion(item: string, keyword: string) {
  ...
  if (normalizedItem.includes(normalizedKeyword)) return 100;
...
function makeThemeBasedTitleSuggestions(theme: string): string[] {
  const t = theme.trim();
  if (!t) return THEME_TITLE_SUGGESTIONS;
...
export default function InputForm({
  ...
  const themeSuggestions = useMemo(() => {
    const merged = [...makeThemeBasedTitleSuggestions(value.theme), ...themeHistory, ...THEME_TITLE_SUGGESTIONS];
